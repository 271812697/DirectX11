...
{
    float3 color = ping[coord].rgb * weight[0];
    for (int i = 1; i < 6; i++) {
        uint2 offset = uint2(i, 0);
        color += ping[ coord + offset].rgb * weight[i];
        color += ping[ coord - offset].rgb * weight[i];
    }
    pong[coord] = float4(color, 1.0);
}

void GaussianBlurV(const uint2 coord)
{
   

    float3 color = pong[coord].rgb * weight[0];
    for (int i = 1; i < 6; i++) {
        uint2 offset = uint2(0, i);
        color += pong[coord + offset].rgb * weight[i];
        color += pong[coord - offset].rgb * weight[i];
    }
    ping[coord] =float4(color, 1.0);
}
[numthreads(32, 18, 1)]
void main(uint3 DTid : SV_DispatchThreadID)
{
    uint2 ils_coord = DTid.xy;
