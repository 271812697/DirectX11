...
Texture2D albedo_map : register(t20);
Texture2D normal_map : register(t21);
Texture2D metallic_map : register(t22);
Texture2D roughness_map : register(t23);
Texture2D ao_map : register(t24);
Texture2D emission_map : register(t25);
Texture2D displace_map : register(t26);
Texture2D opacity_map : register(t27);
Texture2D light_map : register(t28);
...
uniform bool sample_emission;
uniform bool sample_displace;
uniform bool sample_opacity;
uniform bool sample_lightmap;
uniform bool sample_anisotan;
uniform bool sample_ext_910;
uniform bool sample_ext_911;
/***** physically-based material input properties *****/

// shared properties

float4 albedo; // alpha is not pre-multiplied
float roughness; // clamped at 0.045 so that specular highlight is visible
float ao; // 0.0 = occluded, 1.0 = not occluded
float4 emission; // optional emissive color
float2 uv_scale; // texture coordinates tiling factor
float alpha_mask; // alpha threshold below which fragments are discarded

// standard model, mostly opaque but can have simple alpha blending
float metalness; // should be a binary value 0 or 1
float specular; // 4% F0 = 0.5, 2% F0 = 0.35 (water), clamped to [0.35, 1]
float anisotropy; // ~ [-1, 1]
float3 aniso_dir; // anisotropy defaults to the tangent direction

// refraction model considers only isotropic dielectrics
float transmission; // ratio of diffuse light transmitted through the material
float thickness; // max volume thickness in the direction of the normal
float ior; // air 1.0, plastic/glass 1.5, water 1.33, gemstone 1.6-2.33
float3 transmittance; // transmittance color as linear RGB, may differ from albedo
float tr_distance; // transmission distance, smaller for denser IOR
uint volume_type; // refraction varies by the volume's interior geometry
...
float min2(const float2 v)
{
    return min(v.x, v.y);
}
float max2(const float2 v)
{
    return max(v.x, v.y);
}
...
float min4(const float4 v)
{
    return min(min3(v.xyz), v.w);
}
float max4(const float4 v)
{
    return max(max3(v.xyz), v.w);
}
...
float step3(float a, float x, float b)
{
    return step(a, x) * step(x, b);
}
float2 step3(const float2 a, const float2 x, const float2 b)
{
    return step(a, x) - step(b, x);
}
float3 step3(const float3 a, const float3 x, const float3 b)
{
    return step(a, x) - step(b, x);
}
float4 step3(const float4 a, const float4 x, const float4 b)
{
    return step(a, x) - step(b, x);
}
...
float luminance(const float3 linear_rgb)
{
...
{
    float3 u = fmod(float3(0.0+h * 6.0, 4.0+h * 6.0, 2.0+h * 6.0), 6.0);
    float3 v = abs(u - 3.0) - 1.0;
...
{
    if (s <= 1e-4)
        return float3(v,v,v); // zero saturation = grayscale color
...
float3 hsl2rgb(const float3 hsl)
{
...
float4 pack(float x)
{
    const float4 bit_shift = float4(1.0, 255.0, 255.0 * 255.0, 255.0 * 255.0 * 255.0);
    const float4 bit_mask = float4(1.0 / 255.0,1.0/255.0,1.0/255.0, 0.0);
    float4 v = frac(x * bit_shift);
...
float unpack(float4 v)
{
    const float4 bit_shift = float4(1.0, 1.0 / 255.0, 1.0 / (255.0 * 255.0), 1.0 / (255.0 * 255.0 * 255.0));
...
float random2D(const float2 uv)
{
...
float3 UniformSampleSphere(float u, float v)
{
    float phi = v * PI2;
...
float3 UniformSampleHemisphere(float u, float v)
{
    float phi = v * PI2;
...
float2 UniformSampleDisk(float u, float v)
{
    float radius = sqrt(u);
    float theta = v * PI2;
...
void PoissonSampleDisk(float seed, inout float2 samples[16])
{
    ...
    for (int i = 0; i < n_samples; ++i) {
        samples[i] = float2(cos(angle), sin(angle)) * pow(radius, 0.75); // 0.75 is key
        radius += radius_step;
        angle += angle_step;
    }
}
...
{
    if (alpha >= 1.0)
        return INV_PI; // singularity case when gamma = alpha = 1
    float a2 = alpha * alpha;
...
// Schlick 1994, "An Inexpensive BRDF Model for Physically-Based Rendering"
float3 F_Schlick(const float3 F0, float F90, float HoV)
{
...
float3 Reinhard(float3 radiance)
{
    return radiance / (1.0 + radiance);
}

float3 ReinhardLuminance(float3 radiance, float max_luminance)
{
...
float3 ReinhardJodie(float3 radiance)
{
    float3 t = radiance / (1.0 + radiance);
    float3 x = radiance / (1.0 + luminance(radiance));
    return float3(lerp(x.r, t.r, t.r), lerp(x.g, t.g, t.g), lerp(x.b, t.b, t.b));
}
...
float3 Uncharted2Filmic(float3 radiance)
{
    float exposure_bias = 2.0;
    float3 white_scale = float3(1.0,1.0,1.0) / Uncharted2Partial(float3(11.2,11.0,11.0));
    float3 c = Uncharted2Partial(radiance * exposure_bias);
...
{
    return pow(color, float3(2.2,2.2,2.2)); // component-wise
}

float3 Linear2Gamma(float3 color)
{
    return pow(color, float3(1.0 / 2.2,1.0/2.2,1.0/2.2)); // component-wise
}
...
float3x3 ComputeTBN(const float3 position, const float3 normal, const float2 uv)
{
    /*
    float3 dpx = ddx(position);
    float3 dpy = ddx(position);
    float2 duu = ddx(uv);
    float2 dvv = ddx(uv);

    float3 N = normalize(normal);
    float3 T = normalize(dpx * dvv.x - dpy * duu.x);
...
float3 ComputeRAR(const Pixel px)
{
    ...
    {
        float3 aniso_B = px.anisotropy >= 0.0 ? px.aniso_B : px.aniso_T;
        float3 aniso_T = cross(aniso_B, px.V);
        float3 aniso_N = cross(aniso_T, aniso_B); // anisotropic normal

        // the anisotropic pattern fully reveals at a roughness of 0.25
        float3 N = lerp(px.N, aniso_N, abs(px.anisotropy) * clamp(px.roughness * 4, 0.0, 1.0));
...
{
    float r=0.16 * specular * specular;
    float3 dielectric_F0 = float3(r,r,r);
...
*/
float3 ComputeF0(float ni, float nt)
{
    float x = (nt - ni) / (nt + ni); // x = (eta - 1) / (eta + 1)
    return float3(x * x,x*x,x*x);
}
...
*/
float ComputeDepthBias(const float3 L, const float3 N)
{
...
*/
void EvalRefraction(const Pixel px, out float3 transmittance, out float3 direction)
{
    // spherical refraction
    if (px.volume == 0)
    {
        float3 r_in = refract(-px.V, px.N, px.eta);
        float NoR = dot(-px.N, r_in);

        float m_thickness = px.thickness * px.NoV; // local thickness varies
        float r_distance = m_thickness * NoR;

        float3 T = clamp(exp(-px.absorption * r_distance), 0.0, 1.0); // Beer–Lambert's law
        float3 n_out = -normalize(NoR * r_in + px.N * 0.5); // vector from the exit to sphere center
        float3 r_out = refract(r_in, n_out, 1.0 / px.eta);
    ...
    else if (px.volume == 1)
    {
        ...
        float3 T = clamp(exp(-px.absorption * r_distance), 0.0, 1.0); // Beer–Lambert's law
        float3 r_out = normalize(r_in * r_distance - px.V * 10.0); // a fixed offset of 10.0
...
float3 EvalSpecularLobe(const Pixel px, const float3 L, const float3 H, float NoV, float NoL, float NoH, float HoL)
{
    ...
    else if (abs(px.anisotropy) <= 1e-5)
    { // non-cloth, isotropic specular BRDF
    ...
    { // non-cloth, anisotropic specular BRDF
...
float3 EvalClearcoatLobe(const Pixel px, float NoH, float HoL, out float Fcc)
{
    float D = D_TRGGX(px.clearcoat_alpha, NoH);
    float V = V_Kelemen(HoL);
    float3 F = F_Schlick(float3(0.04, 0.04, 0.04), HoL) * px.clearcoat; // assume a fixed IOR of 1.5 (4% F0)
    Fcc = F.x;
    return (D * V) * F;
}
...
float3 EvaluateAL(const Pixel px, const float3 L)
{
    ...
    { // additive clearcoat layer
        ...
        float3 Fr_cc = EvalClearcoatLobe(px, NoHcc, HoL, Fcc);
        Lo *= (1.0 - Fcc);
        Lo += (Fr_cc * NoLcc);
...
void InitPixel(inout Pixel px, const float3 camera_pos)
{
    ...
    px.TBN = px._has_tbn ? float3x3(px._tangent, px._binormal, px._normal) :
        ComputeTBN(px._position, px._normal, px.uv); // approximate using partial derivatives

    px.V = normalize(camera_pos - px.position);
    px.N = sample_normal ? normalize(mul(normal_map.Sample(g_SamLinear,px.uv).rgb * 2.0 - 1.0,px.TBN)):px._normal;
    ...
    px.albedo =sample_albedo ? float4(Gamma2Linear(albedo_map.Sample(g_SamLinear, px.uv).rgb), 1.0) : albedo;
    px.albedo.a = sample_opacity ? opacity_map.Sample(g_SamLinear, px.uv).r : px.albedo.a;
    ...
    px.roughness = sample_roughness ? roughness_map.Sample(g_SamLinear, px.uv).r : roughness;
    ...
    if (model.x == 1)
    {
        px.metalness = sample_metallic ? metallic_map.Sample(g_SamLinear, px.uv).r : metalness;
        ...
        px.aniso_T = sample_anisotan ? anisotan_map.Sample(g_SamLinear, px.uv).rgb : aniso_dir;
        px.aniso_T = normalize(mul(px.aniso_T,px.TBN));
    ...
    else if (model.x == 3)
    {
        ...
        px.Ec = float3(1.0,1.0,1.0); // subsurface scattering loses energy so needs no compensation
    }
    ...
    if (model.y == 1)
    {
        ...
        float mix_roughness = lerp(px.roughness, max_roughness, px.clearcoat);
...
float3 EvaluateIBL(const Pixel px)
{
    ...
    float3 AO = px.ao; // diffuse ambient occlusion
    ...
    Fd = irradiance_map.Sample(g_SamLinear, px.N).rgb * px.diffuse_color * (1.0 - E);
    ...
    if (model.y == 1)
    { // additive clear coat layer
        float Fcc = F_Schlick(float3(0.04,0.04,0.04), px.NoV).x * px.clearcoat; // polyurethane F0 = 4%
        Fd *= (1.0 - Fcc);
        Fr *= (1.0 - Fcc);
...
{
    //return float3(1.0,0.0,0.0)+EvaluateAL(px, L)*0.01;
...
float3 EvaluateAPL(const Pixel px, const float3 position, float range, float linea, float quadratic, float visibility)
{
    ...
    float attenuation = (d >= range) ? 0.0 : (1.0 / (1.0 + 
    linea * d
    +quadratic * d * d));

    return (attenuation <= 0.0 || visibility <= 0.0) ? float3(0.0,0.0,0.0) : (attenuation * visibility * EvaluateAL(px, L));
}

// evaluates the contribution of a white spotlight of unit intensity
float3 EvaluateASL(const Pixel px, const float3 pos, const float3 dir, float range, float inner_cos, float outer_cos)
{
    float3 l = pos - px.position;
...
*/
float EvalOcclusion(const Pixel px, in TextureCube shadow_map, const float3 light_pos, float light_radius)
{
    const float near_clip = 0.1;
    const float far_clip = 100.0;
    float3 l = light_pos - px.position;
    float3 L = normalize(l);
    ...
    float3 U = lerp(float3(1.0, 0.0, 0.0), float3(0.0, 1.0, 0.0), step(abs(L.y), 0.999));
    ...
    { 
     // source: Nvidia 2008, "PCSS Integration whitepaper"
    ...
    for (int i = 0; i < n_samples; ++i)
    {
        ...
        float3 v = -L + (offset.x * T + offset.y * B) * search_radius;
        float sm_depth = shadow_map.Sample(g_SamLinear, v).r;
        if (depth > sm_depth)
        { // in this step we don't need a bias
    ...
    {
        float2 offset = samples[i];
        float3 v = -L + (offset.x * T + offset.y * B) * PCF_radius;
        float sm_depth = shadow_map.Sample(g_SamLinear, v).r;
...
PixelOut main(PixelIn pIn)
{
    ...
    Pixel px;
    px._position = pIn._position;
    px._normal = pIn._normal;
    px._uv = pIn._uv;
    px._uv2 = pIn._uv2;
    px._tangent = pIn._tangent;
    px._binormal = pIn._binormal;
    px._has_tbn = true;
    px._has_uv2 = false;

    InitPixel(px, position.xyz);

    float3 Lo = float3(0.0,0.0,0.0);
   // float3 Le= float3(0.0,0.0,0.0); // emission
