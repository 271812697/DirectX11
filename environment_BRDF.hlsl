...
float3 IntegrateBRDF(float NoV, float roughness, uint n_samples)
{
    ...
    float3 V = float3(sqrt(1.0 - NoV * NoV), 0.0, NoV); // vec3(sin, 0, cos)
    ...
    for (uint i = 0; i < n_samples; i++) {
        ...
        float3 H = ImportanceSampleGGX(u.x, u.y, alpha); // keep in tangent space
        precise float3 L = 2 * dot(V, H) * H - V; // need the precise qualifier
    ...
    for (uint j = 0; j < n_samples; j++) {
        float2 u = Hammersley2D(j, n_samples);
        float3 H = UniformSampleHemisphere(u.x, u.y);
        precise float3 L = 2 * dot(V, H) * H - V;
    ...
    return float3(scale, bias, cloth);
}
RWTexture2D<float4> BRDF_LUT : register(u0);
[numthreads(32, 32, 1)]
void CS(uint3 DTid : SV_DispatchThreadID)
{
    int w, h;
    BRDF_LUT.GetDimensions(w,h);
    float2 resolution = float2(w,h);
    float cosine = float(DTid.x) / resolution.x;
    float roughness = float(DTid.y) / resolution.y;

    float3 texel = IntegrateBRDF(cosine, roughness, 2048);
    BRDF_LUT[DTid.xy] = float4(texel, 0.0);
}
