...
namespace component {
    ...
    {
        float as = D3DApp::Get()->AspectRatio();
        if (reversedZ)
            return XMMatrixPerspectiveFovLH(m_FovY, as, m_FarZ, m_NearZ);
        else
            return XMMatrixPerspectiveFovLH(m_FovY, as, m_NearZ, m_FarZ);
    }
    ...
    void FirstPersonCamera::Pitch(float rad)
    {
        ...
        rotation.x += rad;
        if (rotation.x > XM_PI * 8 / 18)
            rotation.x = XM_PI * 8 / 18;
        else if (rotation.x < -XM_PI * 8 / 18)
            rotation.x = -XM_PI * 8 / 18;
