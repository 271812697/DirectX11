...
namespace component {
    ...
    public:
        ...
        std::string name;
        DirectX::XMMATRIX n2p;   // node space -> parent space (local transform relative to the parent)
        DirectX::XMMATRIX m2n;   // model space (bind pose) -> node space (bone space), for bone nodes only
        DirectX::XMMATRIX n2m;   // bone space -> model space, updated at runtime, N/A if not alive
    };
    ...
    public:
        ...
        //std::unordered_map<unsigned int, asset_ref<Texture2D>>Texture_Diffuse;
